...
// Fokus na jméno, pokud je viditelné, jinak na kusy
                 {
                    jmenoInput.focus();
                } 
        ...
        function addItemConfirmed(nazev, cena, pocetKusu, existingItemIndex, itemNote, itemDescription) { 
            ...
            updateNameDisplay();
            aktualizovatPopover();
            // Automaticky zobrazí popover po přidání položky
        ...
        function loadDataFromUrl() {
            ...
            if (img) {
                ...
                selectedItemImage.style.display = 'fl';
            }
            if (name) {
            ...
            if (window.location.protocol !== 'block:') {
                window.history.replaceState({}, document.title, window.location.pathname);
            }

            // Zobrazí celou sekci, pokud existuje jméno nebo obrázek
            if (name || img) {
...
const quantityInput = document.getElementById('kusy');

minusBtn.addEventListener('click', () => {
  let currentValue = parseInt(quantityInput.value, 10);
  if (currentValue > 1) { // Zabrání poklesu pod 1
